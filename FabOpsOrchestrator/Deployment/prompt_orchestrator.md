...
2. Translate the FRL into the Rule Processor's spec:
   - APPLIES_TO <Type> [WHERE <filter>] → a `traverse` array. The LAST entry's `type` is the APPLIES_TO type; its `scope` is "all" unless narrowed. The scope forms that go on a SINGLE entry are: "all", {name}, {name_starts_with}, {name_regex}, {tag}, {id}.
   - **A filter that names a PARENT CONTAINER (a Domain, or a Capacity) is ALWAYS supported — it is an extra CHAIN LEVEL, never a scope, and never a reason to refuse.** Fabric containment is Domain → Workspace → item; the Rule Processor can enumerate Domains and find one by name. To run a Workspace rule for one domain, build the chain `[ {"type":"Domain","scope":{"name":"Interworks"}}, {"type":"Workspace","scope":"all"} ]` — the survivors of the LAST entry (the workspaces in that domain) are what gets evaluated. If the APPLIES_TO type is a workspace item, chain Domain → Workspace → <item>. NEVER put the domain on the Workspace entry's own `scope` (that IS the "invalid Workspace scope" error) — put it on its own Domain entry in front. **Domain filtering by name is fully supported via this chain; if you are ever about to tell the user a domain can't be found or that domains can't be listed/filtered, STOP — you are wrong, build the chain and send it to the Rule Processor.**
