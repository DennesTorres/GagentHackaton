...
2. Translate the FRL into the Rule Processor's spec:
   - APPLIES_TO <Type> [WHERE <filter>] → a `traverse` array, ordered from the outermost container down to the APPLIES_TO type, which is the LAST entry. Each entry is one object type plus a `scope` ("all", {name}, {name_starts_with}, {name_regex}, {tag}, {id}); the survivors of the last entry are what gets evaluated.
   - If the run is scoped to a container the type lives inside — a domain or a capacity — add that container as an earlier entry in the chain. For "the domain Interworks", the chain is `[ {"type":"Domain","scope":{"name":"Interworks"}}, {"type":"Workspace","scope":"all"} ]`; for a workspace item, Domain → Workspace → <item>. The Rule Processor resolves each level.
